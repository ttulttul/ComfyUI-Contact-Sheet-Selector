...
- When diagnosing “no output” reports it helps to log every selection hand-off; the node now reports sanitized UI selections, warns when they resolve to an empty batch despite incoming data, and falls back to the active set if a stale UI selection points past the new batch bounds.
